...
//火控识别次数
#define vision_time 45
uint32_t vision_time1 = 45;
...
float shoot_fire_debug = 100;
float shoot_fire_stop_debug = 30;
void Shoot::init()
{
    //遥控器指针
    ...
    #if SENTRY_ROBOT
    shoot_rc = communicate.get_rc_yaw_point();
    last_shoot_rc = communicate.get_rc_yaw_point_last();
    #else
    shoot_rc = remote_control.get_remote_control_point();
    last_shoot_rc = remote_control.get_last_remote_control_point();
    #endif
...
bool auto_aim_flag = 0;//火控状态,现在默认打开火控
bool tracking_state_rv2 = 0;
extern uint32_t shoot_detect_count;//在视觉文件中定义的，用于判断是否关闭发射机构
void Shoot::set_mode()
{
    static int8_t last_s = RC_SW_UP; //记录上一次遥控器按键值

    // if (switch_is_up(shoot_rc->rc.s[0]))
    // {
    //     auto_aim_flag = 1;//巡航状态，开火控
    //     /* code */
    // }
...
void Shoot::feedback_update()
{
    ...
    else
    {
        ...
        //   {   
        //           // vision 已经表示连续了
        //         //   if (shoot_mode == SHOOT_STOP && target_detect_count >= shoot_fire_debug) //如果视觉识别到了次数并且，射击模式是停止射击，就设置射击模式是摩擦轮准备
        //         //   {
        //         //     shoot_mode = SHOOT_READY_FRIC;
        //         //     /* code */
        //         //   }
        //         //   if (shoot_mode != SHOOT_STOP && tracking_state_rv2 == 0)
        //         //   {
        //         //     shoot_detect_count ++;
        //         //     if (shoot_detect_count >=500) //一定时间没有识别到目标
        //         //     {
        //         //         shoot_mode = SHOOT_STOP;
        //         //         auto_aim_flag = 0;//关掉火控，在云台控制中，如果识别到了，会把火控打开
        //         //         /* code */
        //         //     }
        ...
        //     //   if(target_detect_count >= shoot_fire_debug && shoot_mode == SHOOT_READY)
        //     //    {
        //     //       shoot_mode = SHOOT_BULLET;
        //     //       //拨弹轮动(表示发射一颗弹丸)
		...
		// 	// 		  //tracking_state_rv2 = FALSE; // 开火后发射置为0
        //     //       }
        //     //     //   if (rad_format(trigger_motor.angle_set - trigger_motor.angle) > 0.05f) //到达发射位置
        //     //     //   {
        //     //     //     shoot_mode = SHOOT_READY; //
        //     //     //     /* code */
        //     //     //   }
        ...
        //   }
          ...
          if (auto_aim_flag == 1)
          {   
            if (shoot_mode == SHOOT_STOP && target_detect_count >= vision_time1)
            {
                shoot_mode = SHOOT_READY_FRIC; //如果捕捉到目标就开启摩擦轮
                /* code */
            }
            if (target_detect_count < vision_time1 && shoot_mode != SHOOT_STOP)
            {
                wating_fire_time++;
                if (wating_fire_time == 1000)
                {
                    shoot_mode=SHOOT_STOP;
                    /* code */
                }
