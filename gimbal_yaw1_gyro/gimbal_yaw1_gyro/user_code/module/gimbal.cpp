...
//extern bool auto_aim_flag;//火控状态
...
void Gimbal::init()
{
    ...
    //遥控器数据指针获取
    #if SENTRY_ROBOT
    gimbal_RC = communicate.get_rc_yaw_point();
    last_gimbal_RC = communicate.get_rc_yaw_point_last();
    #else
...
void Gimbal::gimbal_data_update()
{
    ...
    if (gimbal_mode == GIMBAL_TO_MID)
        ...
        gimbal_yaw_motor.speed = gimbal_INT_gyro_point[INS_GYRO_Z_ADDRESS_OFFSET];
    ...
    if (gimbal_mode == GIMBAL_TO_MID || gimbal_mode == GIMBAL_FREE)
        ...
        gimbal_pitch_motor.speed = gimbal_INT_angle_point[INS_GYRO_Y_ADDRESS_OFFSET];
}
/**
 * @brief          按键数据更新
 * @Author         WSJ
...
void Gimbal::gimbal_chassis_control(fp32 *yaw, fp32 *pitch)
{
    ...
    if(auto_debug_switch == FALSE)
    {
        update_auto_pid();
        auto_debug_switch = TRUE;
    }
            auto_switch = TRUE;
#endif
    ...
    {   auto_aim_flag = 0;
        ...
        can_receive.send_yaw_mode(gimbal_yaw_motor.encode_angle , 0);
        ...
        static int16_t pitch_patrol_direction = 1 , yaw_patrol_direction = 1;
        //判断yaw，pitch，是否到达了最大角度，如果到了就反向巡航
        if (gimbal_pitch_motor.encode_angle_set > 0.2)//正向巡航到极限
        {
            pitch_patrol_direction = -1;
            /* code */
        }
        if (gimbal_pitch_motor.encode_angle_set < -0.02)//正向巡航到极限
        {
            pitch_patrol_direction = 1;
            /* code */
        }
        if (gimbal_yaw_motor.encode_angle_set > 1)//正向巡航到极限
        {
        ...
        //使用程序中自增yaw，pitch
        *yaw = yaw_patrol_direction * GIMBAL_YAW_PARTOL_SPEED;
        *pitch = pitch_patrol_direction * GIMBAL_PITCH_PARTOL_SPEED;
...
void Gimbal::solve()
{
    ...
    {
        //底盘跟随模式下yaw使用陀螺仪控制,pitch使用陀螺仪控制
        //这里哨兵在点头巡航时小头不使用陀螺仪控制但使用陀螺仪获得偏移量，并把便宜量给编码器控制
...
//根据电机正反转,调解电流发送
#if YAW_TURN
    gimbal_yaw_motor.current_give = -(int16_t)(gimbal_yaw_motor.current_set) - (int16_t)f_current; // -(int16_t)(gimbal_yaw_motor.current_set) 
#else
    gimbal_yaw_motor.current_give = (int16_t)(gimbal_yaw_motor.current_set) + (int16_t)f_current; // (int16_t)(gimbal_yaw_motor.current_set) +
#endif
#if PITCH_TURN
    gimbal_pitch_motor.current_give = -(int16_t)(gimbal_pitch_motor.current_set) -(int16_t)G_current;
    //gimbal_pitch_motor.current_give = -G_current;
#else
    gimbal_pitch_motor.current_give = (int16_t)(gimbal_pitch_motor.current_set) + (int16_t)G_current;
    //gimbal_pitch_motor.current_give = G_current;
#endif
...
void Gimbal::pitch_G_compent(void)
{
    pitch_G_F = cos(gimbal_pitch_motor.encode_angle) * pitch_G * pitch_link;
    G_current = pitch_G_F / 0.741 *16384 / 3;
}

void Gimbal::yaw_f_compent(void)
{
    if (gimbal_yaw_motor.speed_set > 0)
    {
        yaw_fric_compent = yaw_fric_compent_firt;
        /* code */
    }
    if (gimbal_yaw_motor.speed_set < 0)
    {
    ...
    f_current = yaw_fric_compent / 0.741 *16384 / 3;
}