...
const RC_ctrl_t * Communicate::get_rc_yaw_point(void)
{
    return &remote_control_yaw;
}
RC_ctrl_t * Communicate::get_rc_yaw_point_last(void)
{
    return &remote_control_yaw_last;
}
...
void Communicate::run()
{
    ...
    {
                if(SendPacketTwist.mode == 1)
                {
                    SendPacketTwist.mode = 0;
                }
                else
                {
                    SendPacketTwist.mode = 1;
                }
                ch4=0;
    }
    //拨轮值判断是否进入小陀螺模式
    ...
    can_receive.send_SendPacket(SendPacketTwist);
    ...
    void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
    {
        ...
        if (hcan == &CAN_1) //接云台CAN 信息
        {
            ...
            case CAN_SHOOT_SPEED_BOARD_COM_ID:
                ...
                detect_hook(BOARD_COM);
                break;
            case CAN_RC_BORAM_YAW_COM_ID:
                can_receive.receive_rc_board_yaw_com(rx_data);
                break;
