...
void Gimbal::set_pid()
{
    error = error*0.6+error_last*0.4;
    set_kp=4000 ;
    set_ki=0 ;
    set_kd=60 ;
    error = gimbal_yaw_motor.gyro_angle_set - gimbal_yaw_motor.encode_angle;
    error_kd=error - error_last;
    error_ki = error+error_last;
    error_last = error;
    gimbal_yaw_motor.current_set = set_kp*error + set_ki*error_ki+set_kd*error_kd; 
}
...
void Gimbal::init()
{
    /*---------------------------遥控器---------------------------------*/
    //遥控器数据指针获取
    gimbal_RC = decision.get_yaw2_ctrl_point();
    last_gimbal_RC = decision.get_yaw2_ctrl_point_last();
    //初始化上一次的遥控器按键值
...
fp32 Gimbal::motor_ecd_to_angle_change(uint16_t ecd, uint16_t offset_ecd)
{
    ...
    if (relative_ecd > KL_HALF_ECD_RANGE)
    {
        relative_ecd -= LK_ECD_RANGE;
    }
    else if (relative_ecd < -KL_HALF_ECD_RANGE)
    {
        relative_ecd += LK_ECD_RANGE;
    }

    return relative_ecd * LK_MOTOR_ECD_TO_RAD;
}
...
//电流控制
    ...
    can_receive.can_cmd_myaw_motor_9025(gimbal_yaw_motor.current_give);
}
/*****************************(C) CALI GIMBAL *******************************/
/**
